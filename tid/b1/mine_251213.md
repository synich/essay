...
main() -> Endpoint::Start()
 ...
 |_ Engine::Init
   |_execvp(python进程)，通过共享内存(/dev/shm/)+有名信号量(sem_open)通信
```
...
Server -> Engine
 -> Scheduler(是否batch，决定queue_wait时间)
  -> BlockManager(分配KV Cache、复用prefix Cache)
   -> executor(发送request给py，由py通过桥接算子下发给ATB/ACL底层算子)
```

每次executor执行一次生成1个新token，所以从Scheduler到executor会循环多次，直到出现停止符或长度截断。