...
ES2016正式引入Promise，2017引入async/await。通过async关键字，把普通函数用Promise包装起来，如果直接调用async函数，得到的当然是Promise对象，如果用await方式调用，就能得到Promise之后的值。await除了和Promise配合，也能处理具有`.then(resolve,reject)`方法的对象，称为thenable。
...
2. async def函数可以直接调用，返回的对象称为coroutine对象，但取不到值。如果始终不执行，Py还会告警`never awaited`
...
7. Py的await可以作用在async def/asyncio.Task/asyncio.Future/任何实现了`__await__`方法的对象上；JS也有thenable的规定
...
除了Erlang/Go，其他大多数语言的异步方案大都采用async方式，这个方式会导致函数*染色*问题，其根本原因是async属于协作式异步模型。有些人说await是yield的语法糖，只能说整个异步方案有点类似，函数执行到yield/await标记的异步点时，编译器知道这里要做栈销毁（类似return），等待下次resume/poll恢复时重新进入函数，然后从堆上恢复栈变量。

| 关键字 | yield | await |
| ---- | ---- | ---- |
| 位置 | 函数体内，类比return | 函数体外，调用者 |
| 配套点 | 显式resume/send | 操作系统隐式poll |
| 定义环境 | 取决于语言：py比较自由，lua显式声明 | 必须标记async |

而async起的是编译器提示器，功能相对较轻。职责说明接下来会返回Future/Promise对象，分析哪些变量要保存在栈上（寄存器不用保留）。

带GC的语言实现async一般会生成闭包，然后用GC保证异步数据能正确恢复。Rust语言为此引入pin概念，从类型语法层面保证异步点的数据保存在堆上，且内存位置不移动，pin和move一起构成了Rust完整的移动语义。
