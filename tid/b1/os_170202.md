...
传统unix的外设平铺在/dev目录下，采用major+minor设备号的方式管理，maj/min机制被后续的BSD/Linux所继承。

由于/dev目录完全平铺而且存在固定分配的问题，linux产生了sysfs+udev+uevent机制动态管理设备。/sys下有完整设备分类和层级结构，udev则负责动态添加和删除设备，至于/dev只不过是/sys的一个平铺映射，更多是为了兼容。

BSD系统由于其明确分离用户态和内核态的哲学，并不采纳/sys目录的设计思路。仍将/dev作为设备管理的主模式，通过sysctl可以了解硬件的更多信息。