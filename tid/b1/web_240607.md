...
缩进风格：amber+gcss，提供基于缩进的简易开发模板，非主流不深入研究。

主流方案：pongo2(jinja2语法)+scss。使用模版引擎有两种方式，`poprint()`动态注入参数，或用内嵌数据机制`data.lua`，对页面进行变量替换。不仅能引用变量，也能调用函数。比如amber使用`#{var}`引用变量，`#{foo()}`调用函数。

由于lua的IO能力偏弱，如果要用好模版引擎，可以用`POST(url, {param}, {header}, "json/url-encode")`方式转发，从而获取注入参数。变相替代`AddReverseProxy`。
