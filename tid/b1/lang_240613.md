...
第一时间安装hex和rebar3(部分erlang包会依赖)。 `mix local.hex` 安装 hex，否则mix不理解hex仓库协议。如果网络不好会提示下载 ez 包离线安装。ez 就是 zip 包，安装后在~/.mix 目录下解压若干个编译后的 beam 文件。rebar 也支持离线下载然后手动装

hex是elixir设计，最终成为整个erlang社区的事实标准，存放在单独的目录`~/.hex`，配置文件也采用.config后缀，内容格式遵照erlang语法
...
不再支持忽略https校验，如果处在企业内网，必须导出内网CA证书加到hex的信任链
