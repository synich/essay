...
tags有文本和二进制两种流派。文本有两种：ctags和etags。ctags诞生于BSD系统，是vim能原生识别的格式，而etags是emacs的附属品。两者生成的文件格式不同，但ctags能生成etags的格式，而且ctags有序速度更快，使用更广，emacs上有插件能识别ctags格式。

二进制先有cscope，2012年后不再开发，由GNU global继续。除了支持调整还支持查引用，功能显然更强。
