...
介绍两个 Helloworld, GUI 框架 + Scintilla 实现常见一个编辑框。基于 FxRuby 的:
...
如果从更底层点的地方开始, 例如 Win32API 和 Carbon, 站稳脚跟学习图形界面编程, 不过你理解事件模型的实现和常见优化手段以后, 就算编辑器不成功, 也可以自己写个游戏引擎玩玩嘛.

然后挑一个 text storage 数据结构，例如 Cocoa 就自己提供了一个 NSTextStorage, 自己造大约有几个主流选项:
